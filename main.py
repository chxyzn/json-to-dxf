...
from dxf_handler import plot_walls
from dxf_handler import plot_lines
from line_split import add_lines
...
msp = doc.modelspace()
doc.layers.add(name="room_border",lineweight=50,color=3).on()
...
for room in list_of_rooms:
    plot_room(room,msp)
    add_lines(room, horizontal_lines,vertical_lines)
    print(room.corners)

arrange_lines(lines,horizontal_lines,vertical_lines)
plot_lines(msp,lines)
